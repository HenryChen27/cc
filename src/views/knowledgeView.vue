  ...
  <div id="topMenu" class="displayText">
      <el-collapse v-model="activeNames" style="border: none;">
        ...
        <el-collapse-item title="LLM对话配置：" name="1">
          ...
          <div class="select">
            <div class="questionText">请选择知识库:</div>
            <el-select
              v-model="value3"
              placeholder="知识库"
              size="middle"
          ...
          <div  class="select">
              <div class="questionText" >匹配知识条数：</div>
              <el-input-number v-model="num2" :min="1"  @change="handleChange" 
              style="width: 160px"
              class="inputNumber"
              />
          </div>
        ...
        <div  class="select">
              <span class="demonstration questionText">知识匹配分数阈值：</span>
              <el-slider v-model="floatNum2" :format-tooltip="formatTooltip2"  />
          </div>
      ...
      setup(props,ctx){
          ...
          onMounted(()=>{
              ...
              .then(data => {
                  ...
                  var llmPrompts=Object.getOwnPropertyNames(data.PROMPT_TEMPLATES.knowledge_base_chat)
              ...
              fetch(store.state.baseURL+'/knowledge_base/list_knowledge_bases/all').then(response => response.json())
                .then(data => {
                    if(data.code==200){
                        ...
                        {
                            var tmp={label:"",value:""};
                            tmp.label=data.data[0][i];
                            tmp.value=data.data[0][i];
                            options3.push(tmp);
                        }
                        value3.value=options3[0].value;
         ...
         function uploadData(){
              ...
              store.commit("knowOption",ret);
  
         }
          function downloadData(){
              dialog=[];
              dialogName=[];
              for(var i=0;i<store.state.knowdialog.length;i++) 
              {
                  var tempDiv = document.createElement('div');
                  tempDiv=store.state.knowdialog[i].cloneNode(true);
                  dialog.push(tempDiv);
                  dialogName.push(store.state.knowname[i]);
              }
        ...
        //   addText("A","出处 [1] [llm/img/大模型推理优化策略-幕布图片-699343-219844.jpg](http://172.26.94.25:8000/knowledge_base/download_doc?knowledge_base_name=samples&file_name=llm%2Fimg%2F%E5%A4%A7%E6%A8%A1%E5%9E%8B%E6%8E%A8%E7%90%86%E4%BC%98%E5%8C%96%E7%AD%96%E7%95%A5-%E5%B9%95%E5%B8%83%E5%9B%BE%E7%89%87-699343-219844.jpg) \n\nMulti-head\nGrouped-query\nMulti-query\nValues\nKeys\n00000000\nQueries\n\n", "出处 [2] [llm/img/大模型技术栈-算法与原理-幕布图片-729151-372321.jpg](http://172.26.94.25:8000/knowledge_base/download_doc?knowledge_base_name=samples&file_name=llm%2Fimg%2F%E5%A4%A7%E6%A8%A1%E5%9E%8B%E6%8A%80%E6%9C%AF%E6%A0%88-%E7%AE%97%E6%B3%95%E4%B8%8E%E5%8E%9F%E7%90%86-%E5%B9%95%E5%B8%83%E5%9B%BE%E7%89%87-729151-372321.jpg) \n\nl/guantizationerror\nj.j:（i+B）\nI/updateweightsinblock\nendfor\nendfor\n\n", "出处 [3] [llm/img/分布式训练技术原理-幕布图片-42284-124759.jpg](http://172.26.94.25:8000/knowledge_base/download_doc?knowledge_base_name=samples&file_name=llm%2Fimg%2F%E5%88%86%E5%B8%83%E5%BC%8F%E8%AE%AD%E7%BB%83%E6%8A%80%E6%9C%AF%E5%8E%9F%E7%90%86-%E5%B9%95%E5%B8%83%E5%9B%BE%E7%89%87-42284-124759.jpg) \n\nY1\nY2\nWE1\nWE2\n(h,v/N)\n知s@猛猿\n(b, s, h)\n(b,S,v/N)\n\n")
        // return;
          const oDiv = document.getElementById('dialogContainer').querySelectorAll(".QA");
          var requestBody={
              "query": textarea1.value,
              "knowledge_base_name":value3.value,
              "top_k": num2.value,
              "score_threshold": floatNum2.value/50,
              "history": [],
              "stream": true,
          ...
          fetch(store.state.baseURL+'/chat/knowledge_base_chat', options)
          .then(response => {
              if(response.ok){
