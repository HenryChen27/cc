<template>
  ...
  > 请新建或选择一个对话</div>
    ...
    <div  
      ...
      >
       <div  style="padding: 10px; letter-spacing: 2px;font-size: 20px;" class="displayText">
        对话列表
    </div>
      ...
      setup(props,ctx){
          ...
          onMounted(()=>{
              ...
              .then(data => {
                  ...
                  var llmPrompts=Object.getOwnPropertyNames(data.PROMPT_TEMPLATES.agent_chat)
         ...
         function uploadData(){
              ...
              store.commit("agentOption",ret);
  
         }
          function downloadData(){
              dialog=[];
              dialogName=[];
              for(var i=0;i<store.state.agentdialog.length;i++) 
              {
                  var tempDiv = document.createElement('div');
                  tempDiv=store.state.agentdialog[i].cloneNode(true);
                  dialog.push(tempDiv);
                  dialogName.push(store.state.agentname[i]);
              }
              console.log(store.state.agentname);
        ...
        if (event.shiftKey&&event.target.id=="spsp") {
          ...
          fetch(store.state.baseURL+'/chat/agent_chat', options)
          .then(response => {
              if(response.ok){
