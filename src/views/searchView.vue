  ...
  <div id="topMenu" class="displayText">
      <el-collapse v-model="activeNames" style="border: none;">
        ...
        <el-collapse-item title="LLM对话配置：" name="1">
          ...
          <div  class="select">
              <div class="questionText">请选择搜索引擎： </div>
              <el-select
              v-model="value3"
              placeholder="搜索引擎"
              size="middle"
            ...
            <div  class="select">
              <div class="questionText" >匹配搜索结果条数：</div>
      ...
      setup(props,ctx){
          ...
          onMounted(()=>{
              ...
              .then(data => {
                  ...
                  var llmPrompts=Object.getOwnPropertyNames(data.PROMPT_TEMPLATES.search_engine_chat)
                  ...
                  value3.value=data.DEFAULT_SEARCH_ENGINE;
              }) .catch(error => {
                  ElMessage({
                          message: '网络错误，配置加载失败',
                          type: 'error',
              ...
              fetch(store.state.baseURL+'/server/list_search_engines',option).then(response => response.json())
              .then(data => {
                  if(data.code==200){
         ...
         function uploadData(){
              ...
              store.commit("searchOption",ret);
  
         }
          function downloadData(){
              dialog=[];
              dialogName=[];
              for(var i=0;i<store.state.searchdialog.length;i++) 
              {
                  var tempDiv = document.createElement('div');
                  tempDiv=store.state.searchdialog[i].cloneNode(true);
                  dialog.push(tempDiv);
                  dialogName.push(store.state.searchname[i]);
              }
              console.log(store.state.searchname);
        ...
        if (event.shiftKey&&event.target.id=="spsp") {
          ...
          var requestBody={
              ...
              "search_engine_name": value3.value,
              "top_k": num2.value,
              "history": [],
              "stream": true,
          ...
          fetch(store.state.baseURL+'/chat/search_engine_chat', options)
          .then(response => {
              if(response.ok){
