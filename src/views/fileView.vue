<template>
  ...
  <div id="topMenu" class="displayText">
      <el-collapse v-model="activeNames" style="border: none;">
        ...
        <el-collapse-item title="LLM对话配置：" name="1">
          ...
          <div class="select">
              <div class="questionText">请选择对话文件： </div>
              <el-select
              v-model="value3"
              placeholder="请先上传文件"
              size="middle"
    ...
    <div  id="upLoad" class="displayText" v-loading="loading"
    element-loading-text="上传文件中请稍后...">
    <el-upload
      class="upload-demo"
      drag
      multiple
    ...
    export default{
      ...
      setup(props,ctx){
         ...
         function uploadData(){
              ...
              store.commit("fileOption",ret);
              
         }
          function downloadData(){
              dialog=[];
              dialogName=[];
              for(var i=0;i<store.state.fileList.length;i++) 
                options3.push({
                    label:store.state.fileList[i].label,
                    value:store.state.fileList[i].value
                });
                if(options3.length) value3.value=options3[options3.length-1].value;
              console.log(options3);
              for(var i=0;i<store.state.filedialog.length;i++) 
              {
                  var tempDiv = document.createElement('div');
                  tempDiv=store.state.filedialog[i].cloneNode(true);
                  dialog.push(tempDiv);
                  dialogName.push(store.state.filename[i]);
              }
        ...
        if (event.shiftKey&&event.target.id=="spsp") {
          ...
          fetch(store.state.baseURL+'/chat/file_chat', options)
          .then(response => {
              if(response.ok){
      ...
      function beforeUpload (file)  {
        ...
        if (file.size > maxSize) {
          ...
          formData.append('prev_id',fileid )
          loading.value=true;
          fetch(store.state.baseURL+'/knowledge_base/upload_temp_docs', {
              method: 'POST',
              body:formData,
  ...
  ::v-deep.el-popper .is-dark {
    max-width: 200px; /* 设置最大宽度以限制提示框的宽度 */
  }
  .toCenter{
      position: absolute;
      top: 50%;
